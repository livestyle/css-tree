...
define(function(require, exports, module) {
	...
	function nextRanges(node, r) {
		var out = [];
		var nextNodeRanges = function(node, r) {
			if (typeof r === 'string') {
				r = node.range(r);
			}
		...
		var nextNodes = function(node, fn) {
	...
	function updateRanges(ranges, fromPos, delta, cmp) {
		cmp = cmp || function(range, key, pos) {
			// return key === 'start' ? range[key] >= pos : range[key] > pos;
			return range[key] > pos;
		};

		if (ranges instanceof Node) {
			ranges = allRanges(ranges);
		}

		ranges.forEach(function(r) {
			if (cmp(r, 'start', fromPos)) {
				r.start += delta;
			}

			if (cmp(r, 'end', fromPos)) {
				r.end += delta;
			}
		});
	}
	...
	Node.prototype = {
		...
		insert: function(node, atIndex) {
			...
			// update source
			var valueRange = this.range('value');
			var insData = node.valueOf();
			// this._source.update(range(insChar, 0), insData);

			// update all tree ranges except current node’s
			// value range since it must be updated carefully
			this._replaceRange(range(insChar, 0), insData);
			// var ranges = allRanges(this.root).filter(function(r) {
			// 	return r !== valueRange;
			// });

			// updateRanges(ranges, insChar, insData.length, function(range, key, pos) {
			// 	if (!range) {
			// 		return false;
			// 	}
			// 	return key === 'start' ? range[key] >= pos : range[key] > pos;
			// });
			// valueRange.end += insData.length;
		...
		_replaceRange: function(range, value, exclude) {
			value = value || '';
			if (typeof range === 'string') {
				range = this.range(range);
			}
			...
			if (delta) {
				var p = parentRanges(this);
				var n = nextRanges(this, range);
				...
				// var ranges = allRanges(this.root);
				// if (exclude) {
				// 	ranges = ranges.filter(function(r) {
				// 		return r !== exclude;
				// 	});
				// }
				// updateRanges(ranges, range.start, delta);
			}
		},
