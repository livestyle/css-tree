...
define(function(require, exports, module) {
	 ...
	 */
	function extractPropertiesFromRange(src, range) {
		var substr = src.substring(range).replace(reSpaceEnd, '');
	...
	return {
		build: function(css) {
			var src = source(css);
			var fullRange = range(0, css);
			var root = new Section(src, {full: fullRange, self: fullRange}, 'root');
