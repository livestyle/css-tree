...
define(function(require, exports, module) {
	...
	/**
	 * Check if given substring contains prefix for
	 * valid single-line comment. This function resolves
	 * a tricky case: double-slash always means comment except
	 * it’s a part of url() function
	 * @param  {String}  str   
	 * @param  {Number}  start 
	 * @param  {Number}  end
	 * @return {Boolean}
	 */
	function isValidCommentPrefix(str, start, end) {
		// if it’s a minifed source, lines could be very long
		start = Math.max(start, end - 300);
		var prefix = str.substring(start, end);
		return !/\burl\([^\)]*$/i.test(prefix);
	}
			...
			while ((ch = stream.next())) {
				if (ch === '/') {
					...
					if (ch2 === '*') { // multiline CSS comment
						...
						if (!isValidCommentPrefix(stream.string, lineStart, stream.pos - 1)) {
							continue;
						}
