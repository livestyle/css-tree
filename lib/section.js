...
/**
 * Finds best insertion point hint for new property in given node
 * @param  {Node} node
 * @return {Number}
 */
function findBestInsertionPoint(node) {
	// if there are sections in given node, use first section
	// index as insertion point
	for (var i = 0, il = node.children.length; i < il; i++) {
		if (node.children[i] instanceof Section) {
			return i;
		}
	}
...
utils.inherit(Section, Node, {
	...
	addProperty: function(name, value, hint) {
		if (typeof hint === 'undefined') {
			hint = findBestInsertionPoint(this);
		}
